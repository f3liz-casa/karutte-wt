...
- **`wt-relay/`** … 透過 L4 リレー／L4 テレメトリの「庭師」（**別ランタイム・別コンテナ**の姉妹
  ...
  設計は `wt-relay/docs/edge-design.md`、karutte 側の連携は `docs/wt-relay-integration.md`。
...
- **wt-relay 連携（透過 L4 リレー裏で動く）**: `:bind`（WG アドレスだけで待つ）、`:keep_alive_interval_ms`
  ...
  [`docs/wt-relay-integration.md`](docs/wt-relay-integration.md)。
...
`mix test` が緑（57 passed）。**実 QUIC で end-to-end が通っている**:
